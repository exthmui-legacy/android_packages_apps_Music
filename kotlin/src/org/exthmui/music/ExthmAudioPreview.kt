 ...
 */
class ExthmAudioPreview : Activity(), OnPreparedListener, OnErrorListener, OnCompletionListener {
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        setContentView(R.layout.exthm_audio_preview)
        ...
        val mAsyncQueryHandler = object : AsyncQueryHandler(contentResolver) {
            override fun onQueryComplete(token: Int, cookie: Any?, cursor: Cursor?) {
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    mediaMetadataRetriever.setDataSource(this@ExthmAudioPreview, mUri)
                    val mediaTitle = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_TITLE)
                    val mediaArtist = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ARTIST)
                    if (idIdx >= 0) {
                    ...
                    if (mediaArtist != null) {
                        ...
                        mTextLine2.text = getString(R.string.exthm_audio_preview_artist_unknown)
                    }
                } else {
        ...
        if (scheme == ContentResolver.SCHEME_CONTENT) {
            if (mUri!!.authority === MediaStore.AUTHORITY) {
                ...
                mAsyncQueryHandler.startQuery(0, null, mUri, arrayOf(MediaStore.Audio.Media.TITLE, MediaStore.Audio.Media.ARTIST),
                        null, null, null)
            } else {
            ...
            val path = mUri?.path
            mAsyncQueryHandler.startQuery(0, null, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, arrayOf(MediaStore.Audio.Media._ID, MediaStore.Audio.Media.TITLE,
                    MediaStore.Audio.Media.ARTIST), MediaStore.Audio.Media.DATA + "=?", arrayOf(path), null)
        } else {
    ...
    private fun updatePlayPause() {
        ...
        if (b != null && mPlayer != null) {
            if (mPlayer!!.isPlaying) {
                b.setImageResource(R.drawable.exthm_audio_preview_pause_ic)
            } else {
                b.setImageResource(R.drawable.exthm_audio_preview_play_ic)
                mProgressRefresher.removeCallbacksAndMessages(null)
            }
        }
    }
    ...
    private class PreviewPlayer : MediaPlayer(), OnPreparedListener {
        private lateinit var mActivity: ExthmAudioPreview
        var isPrepared = false

        fun setActivity(activity: ExthmAudioPreview) {
            mActivity = activity
